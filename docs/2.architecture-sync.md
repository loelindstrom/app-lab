...
Under `auth-v1`, Firebase membership is the enforceable provider-side write boundary for existing rooms. App Lab client/provider
code checks room token hashes and optimistic versions for normal App Lab clients, but Firebase rules do not currently enforce those
checks against a modified or custom client. Current app sharing is therefore full-access bearer sharing for the app rooms: anyone
with the invite can read, write, and forward those rooms. It is not read-only sharing.

The invite preview flow is a read-before-import step, not a local import. It does not save app source or data locally, but under
`auth-v1` it still claims Firebase membership for the invite's source room so it can load and decrypt the app title, description,
and source-room metadata shown in the confirmation dialog.
...
The top-level [Security](../SECURITY.md) document is the canonical public summary. The most important sync secrets are:

- workspace recovery material: whole-workspace bearer material, including owner setup material
- app invite links: full-access bearer material for one app's source/data rooms
- room decrypt secrets: needed to decrypt encrypted room payloads
- room access tokens: used by normal clients for room access and by Firebase auth-v1 claim records

Generated apps must never receive these values.
...
- `pnpm test:firebase-smoke` for real provider and RTDB rules semantics
- `pnpm test:e2e` for local browser workflows that do not need Firebase
- `pnpm test:firebase-e2e` for browser-level sync workflows tagged `@firebase`
